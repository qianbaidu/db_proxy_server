...
func (c *Conn) readHandshakeResponse() error {
	...
	// mysql login
	if success, err := c.H.Auth(c.user, c.Salt, authData); err != nil || success == false {
		return err
	}
	log.Infof("user[%s] login success", c.user)
	// get user db list
	if err := c.H.InitGetUserDbList(); err != nil {
		return err
	}
	log.Info("InitGetUserDbList success")
	pos += authLen
...
func (c *Conn) readDb(data []byte, pos int) (int, error) {
	if c.capability&CLIENT_CONNECT_WITH_DB != 0 {
		...
		pos += len(db) + 1
		if len(db) > 0 {
			if _, err := c.H.UseDB(db); err != nil {
				log.Error("readDb useDb error , ", err)
				return 0, err
			}
		}
	}
...
func (c *Conn) readAuthData(data []byte, pos int) ([]byte, int, int, error) {
	...
	if c.capability&CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA != 0 {
		...
		if authLen == 0 {
			// skip the next \NUL in case the password ireadAuthDatas empty
			pos++
		}
	}
